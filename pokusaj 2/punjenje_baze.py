...
def load_csv_to_db(csv_path):
    ...
    # Punjenje tablice State (bez provjere duplikata)
    states = df.select("state", "fips").distinct().collect()
    for row in states:
        session.add(State(name=row.state, fips=row.fips))  # Direktno dodajemo svaki zapis
    session.commit()


        # Punjenje tablice Disaster (bez provjere duplikata)
    disasters = df.select(
        "disaster_number", "incident_type", "incident_begin_date", "incident_end_date",
        "ih_program_declared", "ia_program_declared", "pa_program_declared",
        "hm_program_declared", "deaths"
    ).collect()
    ...
    declarations = df.select("declaration_title", "declaration_type", "declaration_date", "designated_area", "declaration_request_number", "place_code", "last_refresh", "disaster_number").distinct().collect()
