...
).select("incident_begin_date", "incident_end_date")
...
csv_fema_data = csv_data.select("incident_begin_date", "incident_end_date")
...
window_spec = Window.orderBy("incident_begin_date")  # Redoslijed po deklaraciji
femaDIM = (
    combined_fema_data
    .withColumn("incident_dates_tk", F.row_number().over(window_spec))  # Osigurava redoslijedne ID-eve
