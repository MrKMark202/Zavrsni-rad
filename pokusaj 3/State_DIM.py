...
combined_data = (
    csv_data.select(F.col("name"), F.col("country_name"))  # Preimenujemo 'state' u 'name'
    .union(state_data.select(F.col("name"), F.col("country_name")))  # Spajamo s podacima iz PostgreSQL
